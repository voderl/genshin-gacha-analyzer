...
export const ItemCloud: FC<IItemCloudProps> = ({
  width = 800,
  height = 600,
  dataSource,
}) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const rectCloudRef = useRef<TRectCloudInstance<TRectItem & {
    data: TItem
  }> | null>(null);
  
  const tooltipRef = useRef<HTMLDivElement>(null);

  const renderDataSource: Array<TRectItem & {
    data: TItem
  }> = useMemo(() => {
    ...
    return dataSource.map((item) => {
      return {
        ...
        draw(ctx) {
          ...
          if (itemImage) fillImageRoundRect(ctx, 0, 0, 96, 96, radius, itemImage, 0, 0, 96, 96 );

          ctx.font = `20px ${FONT_FAMILY}`;
          ctx.textBaseline = 'top';
          const countText = `×${item.count}`;
          const textInfo = ctx.measureText(countText);
          fillRoundRect(ctx, 96 - textInfo.width - 4, 96 - 24, textInfo.width + 4, 24, 8,'rgba(255, 255, 255, .7)');
          ctx.fillStyle = '#000';
  ...
  useEffect(() => {
    ...
    const rectCloud = createRectCloud<TRectItem & {
      data: TItem,
    }>({
      canvas: canvas,
      background: '#fdfdfd',
